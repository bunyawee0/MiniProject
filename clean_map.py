...
csv_files = glob.glob("data_map/*.csv")  # Use double backslashes for file paths
...
for csv_file in csv_files:
    ...
    if 'Rank' in df.columns:  # Ensure the Rank column exists
        df['Rank'] = df['Rank'].replace('map\\map', 'Allrank')
...
combined_df.to_csv('map_allranks_combined.csv', index=False)

print("All CSV files have been successfully merged and saved as 'map_allranks_combined.csv'.")
